...
const SequentialPhrasesIcon = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center bg-white border-[2px] md:border-[3px] border-stone-200 rounded-full w-16 h-16 md:w-24 h-24 shadow-[5px_5px_15px_rgba(0,0,0,0.08)] float-animation relative overflow-hidden group">
  ...
  const Header = () => (
    <header className="p-3 md:p-4 bg-white/95 backdrop-blur-md sticky top-0 z-50 border-b border-stone-200 flex justify-between items-center h-16 md:h-20 shadow-sm">
      ...
      <div className="flex items-center gap-2 cursor-pointer group" onClick={() => setView(ViewMode.INTRO)}>
        <Heart className="text-[#880808] fill-[#880808]" size={18} md:size={22} />
      ...
      <div className="flex items-center gap-1.5 md:gap-3">
        <button onClick={() => setView(ViewMode.GALLERY)} title="Mural Personal" className={`p-1.5 md:p-2 rounded-full border border-stone-200 transition-all ${view === ViewMode.GALLERY ? 'bg-[#ffcc00] border-black shadow-md scale-110' : 'bg-white hover:bg-stone-50'}`}>
          <LayoutGrid size={16} md:size={20} className="text-black" />
        </button>
        <button onClick={() => setView(ViewMode.CALENDAR)} title="Calendario" className={`p-1.5 md:p-2 rounded-full border border-stone-200 transition-all ${view === ViewMode.CALENDAR ? 'bg-black text-white border-black' : 'bg-white hover:bg-stone-50'}`}>
          <CalendarIcon size={16} md:size={20} />
        </button>
        <button onClick={() => { setAlarmActive(!alarmActive); if(!alarmActive) { playChime(); alert("✨ Recordatorio activado."); } }} title="Activar Recordatorio" className={`p-1.5 md:p-2 rounded-full border border-stone-200 transition-all ${alarmActive ? 'bg-[#880808] text-white border-[#880808] shadow-inner' : 'bg-white hover:bg-stone-50'}`}>
          <Bell size={16} md:size={20} className={alarmActive ? 'animate-bounce' : ''} />
        </button>
      </div>
    </header>
  );
  ...
  const GalleryView = () => (
    <div className="max-w-5xl mx-auto p-4 md:p-6 space-y-8 md:space-y-12 pb-24 animate-fade-in font-bree relative z-10">
      <div className="flex items-center gap-3 md:gap-4">
        <button onClick={() => setView(ViewMode.INTRO)} className="p-1.5 md:p-2 bg-black text-white rounded-full hover:bg-[#880808] transition-colors shadow-lg"><ChevronLeft size={18} md:size={20} /></button>
      ...
      <section className="space-y-3 md:space-y-4">
        ...
        <div className="h-48 sm:h-64 md:h-[600px] w-full border border-stone-200 shadow-xl overflow-hidden bg-white flex items-center justify-center relative group rounded-xl md:rounded-2xl transform transition-transform hover:scale-[1.005]">
          {coverImage ? (
            <img src={coverImage} className="w-full h-full object-contain" alt="Portada" />
          ) : (
            <div className="flex flex-col items-center gap-2 text-stone-300">
              <ImageIcon size={48} md:size={64} />
          ...
          <button onClick={() => coverFileInputRef.current?.click()} className="absolute bottom-4 right-4 md:bottom-6 md:right-6 bg-[#ffcc00] border-2 border-black p-3 md:p-5 rounded-full shadow-2xl hover:scale-110 transition-transform z-10"><Camera size={20} md:size={28} className="text-black" /></button>
        </div>
      </section>
          ...
          {Array.from({ length: 31 }).map((_, idx) => {
            ...
            return (
              <div key={dayNum} onClick={() => { setUploadingDayNum(dayNum); dailyFileInputRef.current?.click(); }}
                className="aspect-square border border-stone-200 shadow-sm bg-white overflow-hidden relative group cursor-pointer hover:scale-[1.05] hover:shadow-lg transition-all rounded-lg md:rounded-xl"
              >
                {img ? (
                  <img src={img} className="w-full h-full object-cover" alt={`Día ${dayNum}`} />
                ) : (
                  <div className="w-full h-full flex flex-col items-center justify-center gap-1 md:gap-2 opacity-30 bg-stone-50">
                    <Camera size={18} md:size={24} className="text-black" />
  ...
  const IntroView = () => (
    <div className="relative min-h-[calc(100vh-64px)] md:min-h-[calc(100vh-80px)] z-10 flex flex-col items-center justify-center p-4 md:p-12 animate-fade-in font-bree">
      ...
      <div className="relative z-10 text-center space-y-6 md:space-y-8 max-w-5xl w-full">
        ...
        <h2 className="text-2xl sm:text-3xl md:text-5xl font-black text-[#ffcc00] leading-tight uppercase tracking-tighter drop-shadow-lg px-2">
          Camino de<br className="md:hidden"/> Transformación
        </h2>

        <div className="bg-white/90 backdrop-blur-sm p-5 md:p-12 border border-stone-200 shadow-2xl relative overflow-hidden transform md:-rotate-1 group rounded-[1.5rem] md:rounded-[2.5rem] transition-all hover:rotate-0 hover:bg-white/100">
          <div className="space-y-4 md:space-y-6 text-left text-black leading-relaxed text-base sm:text-lg md:text-2xl">
            ...
            <div className="pt-4 md:pt-6 border-t border-stone-100 space-y-3 md:space-y-4">
              ...
              <div className="flex flex-col gap-1 md:gap-2">
                ...
                <span className="text-3xl sm:text-4xl md:text-7xl font-black text-[#880808] leading-none uppercase tracking-tighter">YO SOY YO</span>
            ...
            <div className="mt-4 md:mt-8 bg-[#ffcc00]/10 text-stone-900 p-4 md:p-6 border-2 border-[#ffcc00] shadow-sm rounded-xl md:rounded-2xl">
               <p className="text-xl sm:text-2xl md:text-3xl font-black text-[#880808] uppercase tracking-tighter mb-0.5 md:mb-1">Yo Soy yo:</p>
               <p className="text-sm sm:text-base md:text-xl font-bold uppercase tracking-widest leading-snug">Completo/a, presente, vivo en mi poder.</p>
            </div>
          </div>
        </div>
  ...
  const CalendarView = () => (
    <div className="max-w-5xl mx-auto p-4 md:p-6 space-y-8 md:space-y-12 pb-24 animate-fade-in font-bree relative z-10">
      <div className="flex items-center gap-3 md:gap-5 mb-6 md:mb-10">
        <button onClick={() => setView(ViewMode.INTRO)} className="p-2 md:p-3 bg-black text-white rounded-full shadow-lg hover:scale-110 transition-transform"><ChevronLeft size={20} md:size={24} /></button>
        <div className="flex flex-col">
        ...
        {MONTH_NAMES.map((month, mIdx) => {
          ...
          return (
            <div key={month} className="space-y-4 md:space-y-8 border-l-[4px] md:border-l-[6px] border-[#ffcc00] pl-4 md:pl-8 scroll-reveal">
              <div className="flex justify-between items-center border-b border-stone-100 pb-2 md:pb-4 bg-white/40 p-3 md:p-5 shadow-sm rounded-tr-[1.5rem] md:rounded-tr-[2rem] backdrop-blur-sm">
                <h3 className="text-2xl sm:text-3xl md:text-5xl font-black text-black uppercase italic tracking-tighter">{month}</h3>
                <Sparkles className="text-[#ffcc00] animate-pulse" size={24} md:size={32} />
              </div>
                ...
                {Array.from({ length: daysInMonth }).map((_, dIdx) => {
                  ...
                  return (
                    <button key={dIdx} onClick={() => dIdx < DAYS_CONTENT.length && selectDay(dIdx, mIdx)} 
                      className={`aspect-square border flex flex-col items-center justify-center transition-all overflow-hidden relative shadow-sm rounded-lg md:rounded-2xl ${dIdx < DAYS_CONTENT.length ? 'bg-white border-stone-100 hover:bg-[#ffcc00]/20 hover:border-[#ffcc00] hover:scale-110 z-10 hover:shadow-xl' : 'bg-stone-50 opacity-10'}`}
                    >
    ...
    return (
      <div className="max-w-screen-xl mx-auto p-3 md:p-8 space-y-8 md:space-y-12 pb-48 animate-fade-in relative font-bree z-10">
        ...
        <div className="relative scroll-reveal max-w-6xl mx-auto">
          ...
          <div className="overflow-hidden min-h-[40vh] sm:min-h-[60vh] md:h-[85vh] shadow-2xl border-[6px] md:border-[10px] border-white relative group bg-white rounded-[2rem] md:rounded-[4rem] transform transition-all duration-1000">
            ...
            <div className="absolute bottom-8 left-8 right-8 md:bottom-16 md:left-16 md:right-16 flex flex-col gap-4 md:gap-8">
               <h2 className="text-2xl sm:text-4xl md:text-6xl font-black text-[#ffcc00] uppercase tracking-tighter leading-tight drop-shadow-lg">{currentDay.title}</h2>
               <button onClick={() => { setUploadingDayNum(currentDayNum); dailyFileInputRef.current?.click(); }} 
                  className="bg-[#ffcc00] text-black px-6 md:px-12 py-3 md:py-5 border-2 border-black flex items-center justify-center gap-3 md:gap-5 text-[10px] md:text-sm uppercase font-black w-fit shadow-xl hover:scale-105 active:scale-95 transition-all rounded-full"
               >
                 <Camera size={20} md:size={30} /> <span>Cambiar Foto</span>
               </button>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-12 md:space-y-20 max-w-5xl mx-auto">
          <div className="bg-white p-6 sm:p-12 md:p-24 border border-stone-100 shadow-xl text-lg sm:text-2xl md:text-4xl leading-relaxed text-[#ffcc00] relative font-medium italic rounded-[2rem] md:rounded-[3rem]">
            <div className="absolute -top-4 md:-top-7 left-8 md:left-16 bg-[#ffcc00] text-black h-8 md:h-14 w-48 md:w-80 border-2 border-black text-[7px] md:text-xs font-black uppercase z-10 shadow-lg overflow-hidden flex items-center rounded-lg">
              <div className="animate-marquee whitespace-nowrap">
                <span className="px-2 md:px-4 tracking-[0.1em] md:tracking-[0.2em]">{mantraText}</span>
                <span className="px-2 md:px-4 tracking-[0.1em] md:tracking-[0.2em]">{mantraText}</span>
              </div>
            </div>
            {currentDay.content.map((p, i) => {
              if (highlightedDays.includes(currentDayNum) && i === 0) {
                return (
                  <div key={i} className="mb-8 md:mb-16 p-4 sm:p-8 md:p-12 bg-white border-[3px] md:border-[6px] border-[#ffcc00] shadow-md transform md:-rotate-1 rounded-2xl md:rounded-3xl relative overflow-hidden">
              ...
              if (currentDayNum === 31 && i === 0) {
                ...
                return (
                  <div key={i} className="mb-6 md:mb-12 p-6 md:p-16 bg-white border-[3px] md:border-[6px] border-[#ffcc00] shadow-lg transform md:-rotate-1 rounded-[2rem] md:rounded-[4rem] relative overflow-hidden">
              ...
              return (
                <div key={i} className="contents">
                  <p className="mb-8 md:mb-12 last:mb-0 first-letter:text-5xl md:first-letter:text-9xl first-letter:font-black first-letter:text-[#ffcc00] first-letter:mr-2 md:first-letter:mr-6 first-letter:float-left leading-tight whitespace-pre-line text-stone-800 text-sm sm:text-lg md:text-2xl">
                    {p}
                  </p>
                  {currentDayNum === 13 && i === currentDay.content.length - 1 && (
                    <div className="mt-8 md:mt-16 mb-8 md:mb-12 p-6 sm:p-10 md:p-16 bg-white border-[3px] md:border-[6px] border-[#ffcc00] shadow-2xl rounded-[2rem] md:rounded-[3rem] relative overflow-hidden">
                      <div className="absolute top-0 right-0 p-4 md:p-8 opacity-10 animate-pulse pointer-events-none">
                        <Flower size={80} md:size={120} className="text-[#880808]" />
                      </div>
          ...
          <div className="bg-white p-8 sm:p-16 md:p-24 border border-stone-100 shadow-2xl relative overflow-hidden group rounded-[3rem] md:rounded-[5rem]">
            <div className="absolute top-0 right-0 p-6 md:p-12 opacity-5 animate-pulse pointer-events-none"><Flower size={150} md:size={300} className="text-[#880808]" /></div>
            <div className="relative z-10 text-stone-900">
              ...
              <div className="relative overflow-hidden w-full mb-8 md:mb-16 py-2 md:py-4">
                <div className="animate-marquee whitespace-nowrap">
                  <span className="text-2xl sm:text-5xl md:text-7xl font-black text-[#ffcc00] uppercase tracking-tighter drop-shadow-sm px-6 md:px-12">"{currentDay.mantra}"</span>
                  <span className="text-2xl sm:text-5xl md:text-7xl font-black text-[#ffcc00] uppercase tracking-tighter drop-shadow-sm px-6 md:px-12">"{currentDay.mantra}"</span>
                </div>
              </div>
              {currentDay.mantraDescription && (
                <p className="text-stone-700 text-base sm:text-3xl md:text-4xl italic border-l-[8px] md:border-l-[15px] border-[#ffcc00] pl-6 md:pl-16 leading-relaxed font-bold whitespace-pre-line">
                  {currentDay.mantraDescription}
                </p>
              )}
            </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 md:gap-10 pt-8 md:pt-16 pb-36 max-w-5xl mx-auto">
          ...
          >
            <div className="flex items-center gap-3 md:gap-6">
              Siguiente <ChevronRight size={30} md:size={60} />
            </div>
            
            <div className="bg-black text-[#ffcc00] border border-black rounded-lg px-2 md:px-4 py-0.5 md:py-1.5 overflow-hidden w-32 md:w-56 h-5 md:h-8 flex items-center shadow-inner">
              <div className="animate-marquee whitespace-nowrap text-[8px] md:text-sm font-black uppercase tracking-[0.1em] md:tracking-[0.15em]">
        ...
        <div className="fixed bottom-6 md:bottom-12 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 md:gap-6 z-50 w-full px-4 md:px-12 max-w-3xl">
          <button onClick={() => { if(alarmActive) playChime(); alert("✨ PRÁCTICA DE HO'OPONOPONO:\n\nRepite el mantra 3 veces.\n\nEl cambio comienza en tu interior."); }} 
            className="bg-white text-[#880808] px-6 md:px-14 py-6 md:py-12 shadow-[0_15px_50px_rgba(0,0,0,0.15)] font-black flex items-center gap-4 md:gap-12 border-2 border-stone-100 w-full justify-center transform hover:scale-105 active:scale-95 transition-transform rounded-full"
          >
    ...
    <div className="min-h-screen bg-stone-50 pb-20 selection:bg-[#ffcc00] selection:text-black relative overflow-x-hidden">
      ...
      <footer className="mt-16 md:mt-32 text-center text-black p-12 md:p-24 border-t border-stone-100 bg-white/95 flex flex-col items-center gap-6 md:gap-12 font-bree relative z-10">
        <div className="flex items-center gap-6 md:gap-16">
          <Heart size={32} md:size={64} className="text-[#880808] fill-[#880808]" />
          <Flower size={40} md:size={80} className="text-[#ffcc00] animate-spin-slow" />
          <Heart size={32} md:size={64} className="text-[#880808] fill-[#880808]" />
        </div>
